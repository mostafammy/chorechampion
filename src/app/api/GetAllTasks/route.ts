    ...
    try {
        ...
        const tasks = await Promise.all(
            taskIds.map(async (taskId) => {
                ...
                if (typeof taskData === 'string') {
                    ...
                    console.log(`Unexpected data type for task ${taskId}:`, typeof taskData);
                    return null;
                }
            })
        );
        ...
        const validTasks = sanitizedTasks.filter(task => {
            return (
                ...
                typeof task.score === 'number' && !isNaN(task.score) &&
                (task.period === 'daily' || task.period === 'weekly' || task.period === 'monthly') &&
                typeof task.completed === 'boolean' &&
                typeof task.assigneeId === 'string' && task.assigneeId.trim() !== ''
            );
        });

        return new Response(JSON.stringify(validTasks), {status: 200});
